...
> **Never use the `service_role` key** in eshop code. It bypasses all Row Level Security and exposes every organization's data to every other organization. See [Multi-Tenant Architecture](/docs/en/architecture/multi-tenancy) for details.
...
Find your `party_id` in the admin panel at **[Admin → Organizations](/docs/en/admin/parties)** — it is the UUID shown in the organization detail page URL.
...
- **[API Reference](/docs/en/api/reference)** — rendered from the OpenAPI spec

## Related

- [Multi-Tenant Architecture](/docs/en/architecture/multi-tenancy) — how organization isolation works and what not to do
- [Role Hierarchy](/docs/en/users/roles) — which users can access which data
- [Environment Setup](/docs/en/getting-started/environment) — where to find your project URL, anon key, and party_id
